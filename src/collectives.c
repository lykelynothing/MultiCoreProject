...
int RingAllreduce(int my_rank, int comm_sz, size_t dim, int algo, float* my_numbers){
  ...
  //uint16 are used to allow adding of quantized number and not overflow
  ...
  //!!!send and recv for homom_quant are NOT NEEDED since all of the processes have the same metadata!!! 
  ...
  for (int i = 0; i < comm_sz - 1; i++){
    ...
    //TODO: this is for Homomorphic compression, need to write also for non Homomorphic compression
  ...
  my_numbers = HomomorphicDequantization(output, struct_ptr->min, struct_ptr->max, comm_sz, dim);
  return MPI_SUCCESS;
}
...
void * Quantize(float * sendbuf, int count, int algo){ 
  ...
  switch(algo){
    ...
    case 2:
      ...
      break;
    case 3:
      struct_ptr = HomomorphicQuantization(sendbuf, count);
      break;
    default:
...
void DequantizeVector(void * struct_ptr, float ** dequantized, int algo, int dim){
  switch (algo){
    case 0:
      struct lloyd_max_quant *str = (struct lloyd_max_quant *)struct_ptr;
      *dequantized = LloydMaxDequantizer(str, dim);
      break;
    case 1:
      struct non_linear_quant *str = (struct non_linear_quant *)struct_ptr;
      *dequantized = NonLinearDequantization(str, dim);
      break;
    case 2:
      struct unif_quant *str = (struct unif_quant *)struct_ptr;
      *dequantized = UniformRangedDequantization(str, dim);
      break;
    default:
      printf("ERROR!! Quant algo not valid\n");
      break;
